...
For production use, consider wrapping `readFolder()` and other critical calls
in try/catch via a custom error handler or testing the file/directory state
beforehand.
...
1. **No tests exist** — The library has no test suite. Add tests before
   relying on it in production.

2. **Web-only downloads** — `download()` and its variants call `header()` and
   `readfile()`. They will fail in CLI contexts.

3. **MIME detection bug in `download()`** — When `$type = 'detect'`, the code
   calls `mime_content_type($filepath)` with the path *only*, not the full
   file path. This is likely a bug — the detected MIME will always be
   `directory` or `false`. Consider always passing an explicit MIME type.

4. **`deleteFile()` uses `unlink()` without error suppression** — Unlike most
   methods, `deleteFile()` calls `unlink()` directly. If the file does not
   exist or is not writable, it emits a PHP warning (but does not `die()`).

5. **`@` error suppression** — The widespread use of `@` means internal PHP
   warnings are hidden. Debugging failures can be difficult.

6. **`$data` includes `.` and `..`** — `readFolder()` stores `scandir()` output
   directly, which includes `.` and `..` entries. Filter them if needed:

   ```php
   $files = array_diff($fm->data, ['.', '..']);
   ```

7. **ZIP flattens paths** — `createZip()` uses `basename()` for each file, so
   nested directory structures are lost inside the archive.

8. **`deleteTree` is recursive and destructive** — There is no confirmation
   or trash mechanism. Double-check the path before calling.
